...
void math_init(t_cub *cub, float new_angle)
{
...
void skip_first_tile(t_cub *cub, float new_angle)
{
    math_init(cub, new_angle);
    if (cub->ray->x_dir < 0)
        cub->ray->x_side_dis = (cub->player->x - cub->ray->curr_x * TILE_SIZE) / -cub->ray->x_dir;
    else
        cub->ray->x_side_dis = ((cub->ray->curr_x + 1) * TILE_SIZE - cub->player->x) / cub->ray->x_dir;
    if (cub->ray->y_dir < 0)
        cub->ray->y_side_dis = (cub->player->y - cub->ray->curr_y * TILE_SIZE) / -cub->ray->y_dir;
    else
        cub->ray->y_side_dis = ((cub->ray->curr_y + 1) * TILE_SIZE - cub->player->y) / cub->ray->y_dir;
    cub->ray->side = 1;
...
void ray_casting(t_cub *cub, float new_angle)
{
    ...
    {    
        ...
        if (is_out_of_bound(cub->map, cub->ray->curr_x, cub->ray->curr_y))
            break ;
        if (strchr("1 \0", cub->map->map[cub->ray->curr_y][cub->ray->curr_x]))
        {
