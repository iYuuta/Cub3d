...
static char	*get_value(char *str, t_cube *cube)
{
	char	*end;

	if (*str && !ft_isspace(*str))
		return (ft_error("Need space between element and value", cube), NULL);
	while (*str && ft_isspace(*str))
		str++;
	if (!*str)
		return (ft_error("Empty Element value", cube), NULL);
...
static int	add_element(char *str, t_cube *cube)
{
	...
	if (dup[0] > 1 || dup[1] > 1 || dup[2] > 1 || dup[3] > 1 || dup[4] > 1
		|| dup[5] > 1)
		ft_error("Duplicate Element", cube);
	if (!ft_strncmp(str, "NO", 2))
		return (dup[0]++, str += 2, cube->no.name = get_value(str, cube), 1);
	else if (!ft_strncmp(str, "SO", 2))
		return (dup[1]++, str += 2, cube->so.name = get_value(str, cube), 1);
	else if (!ft_strncmp(str, "WE", 2))
		return (dup[2]++, str += 2, cube->we.name = get_value(str, cube), 1);
	else if (!ft_strncmp(str, "EA", 2))
		return (dup[3]++, str += 2, cube->ea.name = get_value(str, cube), 1);
	else if (!ft_strncmp(str, "F", 1))
		return (dup[4]++, str++, cube->f = get_value(str, cube), 1);
	else if (!ft_strncmp(str, "C", 1))
		return (dup[5]++, str++, cube->c = get_value(str, cube), 1);
...
void	init_texture(t_cube *cube)
{
	...
	if (!cube->no.texture)
		ft_error("Failed to load the NO texture file", cube);
	...
	if (!cube->so.texture)
		ft_error("Failed to load the SO texture file", cube);
	...
	if (!cube->we.texture)
		ft_error("Failed to load the WE texture file", cube);
	...
	if (!cube->ea.texture)
		ft_error("Failed to load the EA texture file", cube);
